...
//-----------------------------------------------------------------------Eigen 100000 time
    double eigen_100000_time(){
        int k100=100000;
        Eigen::SparseMatrix<double> great_a(k100,k100);
        for(int i=0;i<k100;i++)
            for(int j=0;j<k100;j++){
                if(i==j)
                    great_a.insert(i,j)=1;
            }


        Eigen::SparseVector<double> great_b(k100);

        for(int j=0;j<k100;j+=33000){
            great_b.insert(j) = 0.5;
        }

        Eigen::SparseLU <Eigen::SparseMatrix<double> > solver;
        solver.analyzePattern(great_a);
        solver.factorize(great_a);

        Eigen::SparseVector<double> great_x(k100);
         clock_t begin = clock();
        great_x = solver.solve(great_b);
        clock_t end = clock();
